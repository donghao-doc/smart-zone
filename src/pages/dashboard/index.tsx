...
type EnergyChartOption = {
  ...
  toolbox: { feature: { saveAsImage: Record<string, never> } }
...
function Dashboard() {
  const initalOption: EnergyChartOption = {
    title: {
      text: '当日能源消耗'
    },
    tooltip: {
  ...
  const [data, setData] = useState<EnergyChartOption>(initalOption)
  
  useEffect(() => {
    const loadData = async () => {
      try {
        const { data: apiData } = await getEnergyData()
        const dataList: EnergySeries[] = apiData.map((item) => ({
          name: item.name,
          data: item.data,
          type: 'line',
          stack: 'Total',
